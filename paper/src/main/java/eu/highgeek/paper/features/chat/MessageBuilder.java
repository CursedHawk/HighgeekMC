...
import eu.highgeek.paper.utils.VersionHandler;
...
public final class MessageBuilder {
    ...
    public static Component createChatComponent(ChatMessage message){
    ...
    public static String convertToJson(ChatMessage message) {
        ...
        if(message.isHasItem()){
            ItemStack itemStack = ItemStackUtils.fromString(CommonMain.getRedisManager().getStringRedis(message.getItemAddress()));
            json = injectHoverToJson(json, "[item]", itemStack);
        }

        CommonMain.getLogger().debug("json is: " + json);
        return json;
    }


    public static String injectHoverToJson(String json, String targetWord, ItemStack item){
        Component original = GsonComponentSerializer.gson().deserialize(json);


        Component modified = original.replaceText(builder -> builder
                .matchLiteral(targetWord)
                //set item name
                .replacement(item.displayName()
                        .hoverEvent(item))
        );
        CommonMain.getLogger().debug("original component is: " + original);
        CommonMain.getLogger().debug("modified component is: " + modified);
        CommonMain.getLogger().debug("modified component json is: " + GsonComponentSerializer.gson().serialize(modified));
        return  GsonComponentSerializer.gson().serialize(modified);
    }

    public static String channelMessageBuilder(ChatMessage message){
